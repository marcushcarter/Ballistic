...
Error ImGuiTextureCache::create(drivers::RenderingDeviceDriverVulkan& r_device_driver)
{
    using enum Error;
    ...
    sampler_desc.name = "imgui_preview_sampler";
...
void ImGuiTextureCache::retire(VkImageView p_view, uint64_t frame_number, uint32_t frame_count)
{
    if (p_view == VK_NULL_HANDLE) return;
    auto it = entries.find(p_view);
    if (it == entries.end()) return;
    retired.push_back({ it->second.set, frame_number + frame_count });
    entries.erase(it);
}

void ImGuiTextureCache::collect(uint64_t frame_number, uint32_t frame_count)
{
    (void)frame_count;
    // free retired sets whose in-flight frames have drained
    for (size_t i = 0; i < retired.size();) {
        if (frame_number >= retired[i].retire_frame) {
